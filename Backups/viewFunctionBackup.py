...
from attandenceDashBoard.models import Attandence, StoreFaces ,EmployeeRegistration
...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            return redirect('success_page')  # Redirect to login page or any other page
    else:
...
def register_faces(request):
    ...
    return render(request, 'register_faces.html',{'employees':employees})
...
def markAttandence(request): 
    ...
    if request.method == 'POST':
        ...
        image1.save("static/pp4.jpg") 

        siginImage = save_base64_image(clickedImg) 

        # name = ImageAuthenticationBackend.classify_face3('static/pp4.jpg')   this is for without id 
        # name = DeepFaceLibrary.predict_face("static/pp4.jpg",empId)
        name = FaceAuthentication.classify_face3('static/pp4.jpg',empId)
        print(name)
        
        names = []
        if name:
            emp = EmployeeRegistration.objects.get(empId=name['empId'])
            ...
            if actionType == 'signin':
                ...
                # elif latecount <2 or current_time < target_time:
                    ...
                    names.append(f"{name['name']} - Signed In")
                # else : 
                #     names.append(f"{name['name']} -You are late ")
                ...
                if obj :
                    ...
                    if not obj.singoutTime :
                        ...
                        obj.save()
                        names.append(f"{name['name']} - Signed Out")
                    # elif (eighthourcount <2 ) or todayshours >= eight_hours : 
                        ...
                        names.append(f"{name['name']} - Signed Out")
                    # else : 
                    #     names.append(f"{name['name']} 8 hours is not completed remaining {remianing}")
                else:
                    names.append(f"{name['name']} - No Sign In record found")
...
def attendance_list(request):
    ...
    selected_date = request.GET.get('date')  # Get date from request (from calendar input)
    
    if selected_date:
        try:
            selected_date = datetime.strptime(selected_date, "%Y-%m-%d").date()
        except ValueError:
            selected_date = datetime.today().date()  # If invalid date, default to today
    else:
        selected_date = datetime.today().date()  # Default to today if no date selected
...
# ---------------------------
            ...
            if action_type == 'signin':
                ...
                if obj :
                        ...
                        obj.siginImage = siginImage
                        obj.save()
                        return JsonResponse({'name': result['name'], 'Present': True, 'message': f'signed in and in range {distance}'})
...
def export_month_attendance_to_excel(request):
    try:
        ...
        def generate_table_data(day_range):
            ...
            for employee in EmployeeRegistration.objects.all():
...
# @login_required()
# @user_passes_test(is_admin)
# def getmonthsData(request):
#     return render(request,'monthsData.html')
...
def login_view(request):
    ...
    if user is not None :
        ...
        login(request, user)
        next_url = request.GET.get('next', '/oas/')  # Default to home if no next
