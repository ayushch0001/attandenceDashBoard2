...
class ImageAuthenticationBackend:
    ...
    def read_uploaded_image(uploaded_file): # type: ignore
        # Read the file bytes
        file_bytes = np.asarray(bytearray(uploaded_file.read()), dtype=np.uint8) # type: ignore
        # Decode into an OpenCV image
        img = cv2.imdecode(file_bytes, cv2.IMREAD_COLOR)
        return img
    
    
    def authenticate(profile_picture,empId): # type: ignore
        try:
            ...
            for p in qs:
                for img_path in [p.front, p.left, p.right]:  
                    ...
                    profile_picture = profile_picture.replace("\\", "/") # type: ignore
