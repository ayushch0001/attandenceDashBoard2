...
from attandenceDashBoard.models import Attandence, Department, Failedattandence, LeaveManagement, StoreFaces ,EmployeeRegistration
...
from .salaryServices import AttandenceService , SalaryServices
...
def markAttandence(request): 
    ...
    if request.method == 'POST':
        ...
        try :
            ...
            if name:
                ...
                try:
                    ...
                    str =  f"No employee found with empId: {empId}"
                    names = None
                    names = []
                    names.append(str)
                    return JsonResponse({'namesList': names})
                ...
                newFailedAttempt = Failedattandence.objects.create(
                            emp=emp,
                            date = today,
                            singInTimeA=current_time,
                            siginImageA=siginImage )
                newFailedAttempt.save()
                names = None
        except EmployeeRegistration.DoesNotExist:
                    str = f"No employee found with empId: {empId}"
                    names = []
        ...
        return JsonResponse({'namesList': f"Tring with the wrong id check for that {empId}"})
...
@csrf_exempt  # Remove this if using CSRF token correctly in JS 
@require_POST
def leaveManagement(request):
    try:
        ...
        if not emp_id:
            return JsonResponse({"success": False, "message": "Employee ID not provided"}, status=400)

        emp = EmployeeRegistration.objects.get(id=emp_id)
        today = now().date()
...
@csrf_exempt  # Optional — better to pass CSRF token in JS
...
def failedAttem_list(request):
    if request.method == "POST":
        ...
        if selected_date:
            ...
            selected_date = datetime.today().date() 
        if str(departId) != "0":
            attemepted = Failedattandence.objects.filter(date=selected_date,emp__deprt=departId).order_by('-singInTimeA')
        else :
            attemepted = Failedattandence.objects.filter(date=selected_date).order_by('-singInTimeA')
        return render(request, 'failedAttemts.html', {
            'attendances': attemepted,
            'selected_date': selected_date,
            'deparments' : deparments,
            'no_records': not attemepted.exists()
        })
...
@csrf_exempt
def getSalaryOfEmployee(request):

    if request.method == "POST":
        empId = request.POST.get('empId')
        month = request.POST.get('month')

        employee = EmployeeRegistration.objects.filter(empId = empId).first()
        if not employee :
            return redirect('success_page')
    ...
    return render(request, 'salaryDashBoard.html', {
                'message': 'Enter your Id and month',           
            })


def markAllowedLeave(request):
    if request.method == 'POST':
        leaveGiven = request.POST.get('leaveGiven')
        empId = request.POST.get('empId')
        month = request.POST.get('month')
        employee = EmployeeRegistration.objects.filter(empId = empId).first()
        attService = AttandenceService()
        returnV = attService.leaveTaken(employee,leaveGiven,month)
        if returnV :
            return JsonResponse({"success": True, "message": "Leave marked successfully."})
        else :
